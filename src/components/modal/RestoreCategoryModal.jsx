...
const RestoreCategoryModal = ({ id, categoryName }) => {
    ...
    const handleRestore = async () => {
        try {
            ...
            const res = await CategoryServices.restoreCategory(id);
            setIsUpdate(true);
            notifySuccess(res.message || "Khôi phục danh mục thành công!");
            setServiceId();
            closeModal();
        } catch (err) {
            notifyError(err?.response?.data?.message || err?.message || "Không thể khôi phục danh mục!");
        } finally {
            ...
            <Modal isOpen={isModalOpen} onClose={closeModal}>
                <ModalBody className="text-center custom-modal px-8 pt-6 pb-4">
                    ...
                    <h2 className="text-xl font-medium mb-2">
                        {t("Bạn muốn khôi phục")} <span className="text-blue-500">{categoryName}</span>?
                    </h2>
                </ModalBody>
